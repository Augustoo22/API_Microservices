...
type Props = {
  ...
  rowsPerPage?: number;
  onDelete: (id: number) => void; // Função para apagar veículo
};

const TabelaVeiculos: React.FC<Props> = ({ headers, data, rowsPerPage = 5, onDelete }) => {
  ...
  const handleEdit = (id: number) => {
    if (router && id) {
      console.log("Redirecionando para a edição do veículo com ID:", id); // Verifique se o ID está sendo capturado corretamente
      router.push(`/veiculos/editar/${id}`); // Direciona para a página de edição
    }
  };
